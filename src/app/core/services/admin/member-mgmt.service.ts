...
import { IMember, IMembersListResponse } from '../../../shared/models/member.interface';
...
export class MemberMgmtService {
  ...
  private appendDataToFormData(data: IMember, file: File | null): FormData {
    ...
    for (const key in data) {
      if (data.hasOwnProperty(key) && data[key as keyof IMember] !== undefined) {
  ...
  requestRetrieveMembersList(): Observable<IMembersListResponse> {
    return this.httpClient.get<IMembersListResponse>(`${this.api}/RetrieveAllMembers`).pipe(
  ...
  requestUpdateUserStatus(memberId: string): Observable<IErrorResponse> {
    return this.httpClient.put<IErrorResponse>(`${this.api}/updateUserStatus`, { memberId }).pipe(
  ...
  requestEditMemberData(updatedData: IMember, file: File | null): Observable<IErrorResponse> {
    ...
    return this.httpClient.put<IErrorResponse>(`${this.api}/editMemberData`, formData).pipe(
      map(response => response as IErrorResponse),
      catchError(err => throwError(err.message ? err.message : err))
    );
  }
  ...
  requestAddMemberData(newMember: IMember, file: File | null): Observable<IErrorResponse> {
    ...
    return this.httpClient.post<IErrorResponse>(`${this.api}/addMember`, formData).pipe(
      map(response => response as IErrorResponse),
      catchError(err => throwError(err.message ? err.message : err))
    );  
  }
